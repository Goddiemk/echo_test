...
function sqlite3SampleCode(resultsCallback) {
  ...
  db.serialize(function() {
    ...
    db.all("SELECT rowid AS id, info FROM lorem", function(err, rows) {
      var result = "";
      rows.forEach(row => (result += row.id + ": " + row.info + "\n"));
...
rn_bridge.channel.on("message", msg => {
  try {
    switch (msg) {
      case "versions":
        rn_bridge.channel.send("Versions: " + JSON.stringify(process.versions));
        break;
      case "sha3":
        rn_bridge.channel.send("sha3 output:\n" + sha3SampleCode());
        break;
      case "sqlite3":
        sqlite3SampleCode(result =>
          rn_bridge.channel.send("sqlite3 output:\n" + result)
        );
        break;
      default:
